...
class PlayerController:
	...
	def create_player(self):
		...
		player = self.manager_controller.check_user_input(input_player, message, choices)
	...
	def add_player_to_tournament(self):
		add_player_message = "Souhaitez-vous ajouter 8 joueurs aux tournois ? (o/n) : "
		add_player_response = self.manager_view.prompt_command(add_player_message)
	...
	def add_or_create_player(self):
		...
		max_player_count = 8
		player_count = 1
		create_or_add_message = "Souhaitez-vous créer des joueurs ou en ajouter déjà existant ? (creer/ajouter) : "
		create_or_add_response = self.manager_view.prompt_command(create_or_add_message)
		if create_or_add_response == "créer" or create_or_add_response == "creer":
			...
			successfully_added_message = "Votre joueuse/joueur a bien été ajouté"
			while player_count <= max_player_count:
