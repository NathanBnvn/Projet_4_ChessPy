...
class ReportController:
    ...
    def tournament_players_alphabetical(self):
        ...
        tournament_id = self.manager_controller.check_id_input(
            len(tournaments))
        if tournament_id:
    ...
    def tournament_rounds(self):
        selected_rounds = self.list_rounds()
        if selected_rounds:
            self.manager_view.show_json(selected_rounds)
        self.return_to_menu()

    def tournament_matchs(self):
        rounds = self.list_rounds()
        for round_instance in rounds:
            matchs = round_instance[0]['round']
            self.manager_view.show_json(matchs)
        self.return_to_menu()
